...
from app.health_events.analytics.schemas import Period, TrendsResponse
...
async def get_trends(
    ...
    glucose_unit: GlucoseUnit = Query(GlucoseUnit.mg_dl),
    patient: Patient = Depends(get_current_patient),
    db: AsyncSession = Depends(get_db),
) -> TrendsResponse:
    return await analytics_service.get_trends(db, patient.id, period, glucose_unit)
